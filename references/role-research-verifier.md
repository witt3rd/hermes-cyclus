...
READ-ONLY by contract; the parent skill (cyclus-deep-research T8) also passes a tools allowlist excluding write/edit when `delegate_task` supports per-call tool scoping. Otherwise this contract is enforced by prose only — see Known Gaps in the skill.
