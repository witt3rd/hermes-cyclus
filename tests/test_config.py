"""Tests for cyclus_config.py — _deep_merge and get_config edge cases."""

import pytest
import cyclus.cyclus_config as cyclus_config_module
from cyclus.cyclus_config import _deep_merge, get_config, reload_config
...
def test_get_config_loads_yaml_override(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(cyclus_config_module, "_find_config_file", lambda: cfg_file)
    config = get_config()
...
def test_get_config_no_config_file_returns_empty(monkeypatch):
    monkeypatch.setattr(cyclus_config_module, "_find_config_file", lambda: None)
    config = get_config()
    assert config == {}
