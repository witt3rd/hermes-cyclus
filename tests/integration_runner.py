...
def setup_workdir():
    ...
    for key, mod in sys.modules.items():
        if key.endswith("cyclus_config") and hasattr(mod, "_config_cache"):
...
def _():
    loaded = manager._plugins.get("cyclus")
...
@test("Tools registered: omh_state, cyclus_evidence")
def _():
    assert "omh_state" in manager._plugin_tool_names
    assert "cyclus_evidence" in manager._plugin_tool_names


@test("Tools have correct toolset='cyclus' in registry")
def _():
    from tools.registry import registry
    for name in ["omh_state", "cyclus_evidence"]:
        entry = registry._tools.get(name)
        assert entry is not None, f"{name} not in registry"
        assert entry.toolset == "cyclus", f"{name} toolset={entry.toolset}"
...
@test("cyclus_evidence tool: runs echo command")
def _():
    from tools.registry import registry
    h = registry._tools["cyclus_evidence"].handler
...
@test("cyclus_evidence tool: rejects disallowed commands")
def _():
    from tools.registry import registry
    h = registry._tools["cyclus_evidence"].handler
