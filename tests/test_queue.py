...
class TestPost:
    def test_post_is_idempotent(self, queue_env):
        ...
        task_id_1 = post(mode="loop", instance_id="plan-alpha", kind="TaskExecutionKind", name="Alpha Plan")
        task_id_2 = post(mode="loop", instance_id="plan-alpha", kind="TaskExecutionKind", name="Alpha Plan (dup)")
        assert task_id_1 == task_id_2

    def test_post_different_instance_ids_are_distinct(self, queue_env):
        """Different instance_ids produce different task_ids."""
        task_id_1 = post(mode="loop", instance_id="plan-a", kind="TaskExecutionKind", name="A")
        task_id_2 = post(mode="loop", instance_id="plan-b", kind="TaskExecutionKind", name="B")
        assert task_id_1 != task_id_2

    def test_post_creates_queue_dirs(self, queue_env):
        """The queue directories are created at <project_root>/.cyclus/queue/."""
        post(mode="loop", instance_id="plan-x", kind="TaskExecutionKind", name="X")
    ...
    def test_post_sets_spec_path_to_none(self, queue_env):
        ...
        post(mode="loop", instance_id="plan-null-spec", kind="TaskExecutionKind", name="NS")
        info = status(mode="loop", instance_id="plan-null-spec")
    ...
    def test_post_schema_has_cancel_requested_field(self, queue_env):
        ...
        post(mode="loop", instance_id="plan-cr", kind="TaskExecutionKind", name="CR")
        info = status(mode="loop", instance_id="plan-cr")
        assert info is not None
...
class TestClaim:
    def test_claim_returns_claimed_for_pending(self, queue_env):
        ...
        post(mode="loop", instance_id="pending-test", kind="TaskExecutionKind", name="Pending")
        result = claim(mode="loop", instance_id="pending-test")
    ...
    def test_claim_returns_running_for_held_item(self, queue_env):
        ...
        post(mode="loop", instance_id="held-item", kind="TaskExecutionKind", name="Held")
        first = claim(mode="loop", instance_id="held-item")
        assert first.status == "claimed"
        second = claim(mode="loop", instance_id="held-item", heartbeat_timeout_seconds=300)
    ...
    def test_claim_returns_not_found_for_absent_item(self, queue_env):
        ...
        result = claim(mode="loop", instance_id="does-not-exist")
    ...
    def test_claim_reclaims_timed_out(self, queue_env):
        ...
        post(mode="loop", instance_id="stale-item", kind="TaskExecutionKind", name="Stale")
        first = claim(mode="loop", instance_id="stale-item")
        assert first.status == "claimed"

        # Force the active JSON to have a very old heartbeat
        active_path = queue_env / ".cyclus" / "queue" / "active" / "loop--stale-item.json"
        data = json.loads(active_path.read_text())
        data["last_heartbeat"] = "2000-01-01T00:00:00+00:00"
        active_path.write_text(json.dumps(data))

        second = claim(mode="loop", instance_id="stale-item", heartbeat_timeout_seconds=1)
...
class TestRelease:
    def test_release_allows_immediate_reclaim(self, queue_env):
        ...
        post(mode="loop", instance_id="release-me", kind="TaskExecutionKind", name="Release")
        first = claim(mode="loop", instance_id="release-me")
        assert first.status == "claimed"

        release(mode="loop", instance_id="release-me")

        # After release(), item is PENDING — verify via status()
        info = status(mode="loop", instance_id="release-me")
        assert info is not None
        assert info["status"] == "PENDING"
        assert info["last_heartbeat"] is None

        # A new claim should succeed immediately
        second = claim(mode="loop", instance_id="release-me", heartbeat_timeout_seconds=300)
        assert second.status == "claimed"

    def test_release_noop_on_non_running(self, queue_env):
        """release() on a PENDING item is a safe no-op (idempotent)."""
        post(mode="loop", instance_id="noop-release", kind="TaskExecutionKind", name="NR")
        release(mode="loop", instance_id="noop-release")
        info = status(mode="loop", instance_id="noop-release")
        assert info is not None
        assert info["status"] == "PENDING"
...
class TestCancel:
    def test_cancel_sets_flag(self, queue_env):
        ...
        post(mode="loop", instance_id="cancel-me", kind="TaskExecutionKind", name="Cancel")
        claim(mode="loop", instance_id="cancel-me")
        cancel(mode="loop", instance_id="cancel-me", reason="test request")
        info = status(mode="loop", instance_id="cancel-me")
        assert info is not None
    ...
    def test_cancel_noop_on_missing_item(self, queue_env):
        ...
        cancel(mode="loop", instance_id="ghost-item", reason="test")


class TestWriteState:
    def test_write_state_updates_heartbeat(self, queue_env):
        """write_state() persists data and bumps last_heartbeat each call."""
        post(mode="loop", instance_id="hb-test", kind="TaskExecutionKind", name="HB")
        claim(mode="loop", instance_id="hb-test")

        before = status(mode="loop", instance_id="hb-test")
        hb_before = before["last_heartbeat"] if before else None

        time.sleep(1.1)

        write_state(mode="loop", instance_id="hb-test", state={"task": 3, "phase": "execute"})

        after = status(mode="loop", instance_id="hb-test")
    ...
    def test_write_state_writes_file(self, queue_env):
        ...
        post(mode="loop", instance_id="file-test", kind="TaskExecutionKind", name="File")
        claim(mode="loop", instance_id="file-test")

        payload = {"task_index": 7, "done": False}
        write_state(mode="loop", instance_id="file-test", state=payload)

        info = status(mode="loop", instance_id="file-test")
        assert info is not None
...
class TestComplete:
    def test_complete_marks_terminal(self, queue_env):
        ...
        post(mode="loop", instance_id="done-item", kind="TaskExecutionKind", name="Done")
        claim(mode="loop", instance_id="done-item")
        complete(mode="loop", instance_id="done-item", terminal_state="PlanComplete")
        info = status(mode="loop", instance_id="done-item")
        assert info is not None
...
class TestStatus:
    def test_status_is_readonly(self, queue_env):
        ...
        post(mode="loop", instance_id="readonly-test", kind="TaskExecutionKind", name="RO")
        claim(mode="loop", instance_id="readonly-test")

        info1 = status(mode="loop", instance_id="readonly-test")
        assert info1 is not None
        hb1 = info1["last_heartbeat"]

        time.sleep(1.1)

        info2 = status(mode="loop", instance_id="readonly-test")
    ...
    def test_status_returns_none_for_absent(self, queue_env):
        ...
        result = status(mode="loop", instance_id="absent-item")
        assert result is None

    def test_status_reflects_cancel_requested(self, queue_env):
        """status() after cancel() shows cancel_requested=1."""
        post(mode="loop", instance_id="cancel-status", kind="TaskExecutionKind", name="CS")
        claim(mode="loop", instance_id="cancel-status")
        cancel(mode="loop", instance_id="cancel-status")
        info = status(mode="loop", instance_id="cancel-status")
        assert info is not None
        assert info["cancel_requested"] == 1
...
class TestClaimTerminalItem:
    def test_claim_returns_not_found_for_completed_item(self, queue_env):
        ...
        post(mode="loop", instance_id="already-done", kind="TaskExecutionKind", name="Done")
        claim(mode="loop", instance_id="already-done")
        complete(mode="loop", instance_id="already-done", terminal_state="PlanComplete")
        result = claim(mode="loop", instance_id="already-done")
        assert result.status == "not_found", (
...
class TestReadState:
    def test_read_state_returns_empty_before_write(self, queue_env):
        post(mode="loop", instance_id="rs-test", kind="TaskExecutionKind", name="RS")
        assert read_state(mode="loop", instance_id="rs-test") == {}

    def test_read_state_after_write(self, queue_env):
        post(mode="loop", instance_id="rs-written", kind="TaskExecutionKind", name="RSW")
        claim(mode="loop", instance_id="rs-written")
        write_state(mode="loop", instance_id="rs-written", state={"x": 42})
        assert read_state(mode="loop", instance_id="rs-written") == {"x": 42}


class TestRecordTurn:
    def test_record_turn_appends(self, queue_env):
        post(mode="loop", instance_id="turn-test", kind="TaskExecutionKind", name="TT")
        claim(mode="loop", instance_id="turn-test")
        record_turn(mode="loop", instance_id="turn-test", turn={"n": 1})
        record_turn(mode="loop", instance_id="turn-test", turn={"n": 2})
        history = turn_history(mode="loop", instance_id="turn-test")
...
class TestCounts:
    def test_counts_reflects_queue_state(self, queue_env):
        post(mode="loop", instance_id="count-a", kind="TaskExecutionKind", name="A")
        post(mode="loop", instance_id="count-b", kind="TaskExecutionKind", name="B")
        c = counts()
        assert c["pending"] == 2
        assert c["active"] == 0
        claim(mode="loop", instance_id="count-a")
        c = counts()
        assert c["pending"] == 1
        assert c["active"] == 1
...
class TestGet:
    def test_get_is_alias_for_status(self, queue_env):
        post(mode="loop", instance_id="get-test", kind="TaskExecutionKind", name="GT")
        assert get(mode="loop", instance_id="get-test") == status(mode="loop", instance_id="get-test")
