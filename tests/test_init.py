...
def test_install_skills_cleans_up_tmp_on_error(tmp_path, skill_src):
    ...
    with patch("cyclus.shutil.copytree", side_effect=fail_copytree):
        _install_skills(skill_src, dest_root)
...
def test_register_tools():
    ctx = MagicMock()
    with patch("cyclus._install_skills"):
        from cyclus import register
        register(ctx)
    names = {c.args[0] for c in ctx.register_tool.call_args_list}
    assert names == {"cyclus_queue", "cyclus_evidence"}
