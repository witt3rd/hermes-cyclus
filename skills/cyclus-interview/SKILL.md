...
> **Loop kind:** This skill maps to `ClarificationKind`.  
> `ClarificationKind.HUMAN_GATED = True` — the scheduler structurally cannot mark
> this loop terminal; there is no automated done-criterion. Only an explicit human
> confirmation in Phase 4 ends the interview.  
...
1. Enumerate active interviews by checking for existing state files:
   ```
   glob .omh/state/interview--*.json
   ```
   Each JSON file corresponds to an interview; its `instance_id` and `status` are in the file.
...
3. If resuming: `read_file(".omh/state/interview--{id}.json")` — read round summaries to reconstruct context
