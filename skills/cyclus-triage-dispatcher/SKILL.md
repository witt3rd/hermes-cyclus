...
You are the orchestrator if you are dispatching `cyclus-triage` against a backlog and intending to land verdicts (closures, recasts, version targeting) on real issues.

You are not the orchestrator if you are running `cyclus-triage` as an experiment to see what the skill produces (no execution intent). For experiments, skip the user sign-off gate and just observe — but mark the run as exploratory in any output doc.

## Pre-flight Discipline

Before invoking `cyclus-triage`:
...
If not, propose authoring these first — `cyclus-triage` produces dated docs and project-board updates; if there's no infrastructure to write them into, the output evaporates.
...
Don't write your verdicts into the doc and then run `cyclus-triage` to "validate" them. The roles' job is to surface what the dispatcher missed; if the dispatcher already decided, the roles are theater. Write the doc *from* the run output, not *toward* it.
...
Grooming is not a daily activity. If you find yourself dispatching `cyclus-triage` weekly, the underlying problem is more issues are filed than the team is willing to act on; the fix is upstream (better filing discipline, smaller backlog, fewer new issues), not more triage.

## Templates

- `plugins/omh/skills/cyclus-triage/references/triage-doc-template.md` — structure for the round doc
- `plugins/omh/skills/cyclus-triage/references/orchestrator-review-template.md` — pre-execution review by you
