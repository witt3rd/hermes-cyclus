...
`delegate_task(tasks=[...])` after `cyclus_evidence` produces real
test output.
...
## Evidence (from `cyclus_evidence`, run by the orchestrator)

The orchestrator ran the project's actual test/build/lint commands
BEFORE dispatching you. Real output:

```
<paste the structured evidence object from cyclus_evidence:
...
- **Real evidence output** from `cyclus_evidence`. Without it,
