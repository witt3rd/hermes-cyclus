...
iter 1: pick eligible tasks → dispatch executors (parallel where safe)
        ...
        → update state → write_state → release → exit
...
Pass the role prompt in the executor's context:
```
context=f"## Role\n{executor_prompt}\n\n## Project Context\n{...}"
```
...
### 4. Gather evidence with `cyclus_evidence` BEFORE dispatching verifiers

After all executors complete, run the project's actual test/build/lint
commands via `cyclus_evidence`. This produces a structured
...
**Allowlist gotcha.** `cyclus_evidence` enforces a token-prefix
...
Dispatch with:
- The verifier role prompt in context (see above).
...
Always run `cyclus_evidence` before dispatching verifiers.
...
- Call `cyclus_queue(action="claim", ...)` at iteration start. If `status="not_found"`,
  call `post()` first then `claim()`. If `status="running"`, another session holds it — back off.
- Update state after every executor completion, every verifier
  verdict, every task transition via `cyclus_queue(action="write_state", ...)`.
  Each `write_state()` call updates the heartbeat timestamp automatically.
- Call `cyclus_queue(action="release", ...)` at every clean exit point — success, blocked,
  max-iterations. This resets the item to PENDING so the next invocation can claim immediately.
  Heartbeat auto-reclaims on crash (after 300s); no manual unlock needed.

Do NOT leave items in RUNNING state at a non-clean exit. If a heartbeat times out, the
next ralph invocation will reclaim via `claim()` automatically — no manual intervention.
If `claim()` returns `status="running"` with a fresh heartbeat, report the in-progress
holder to the user; offer to `cancel()` + retry.
...
The fastest way to know is to check your function schema — if the new tool isn't
there, the boot didn't happen.
...
- All tasks have `passes: true` in the task list (in state at `state_path`).
...
- Item marked terminal via `cyclus_queue(action="complete", ..., terminal_state="PlanComplete")`.
...
not the verifier.** Always `cyclus_evidence` BEFORE dispatching
